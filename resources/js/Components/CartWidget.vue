  ...
  <transition name="fade">
    ...
    >
      <div class="cart-widget__content">
        <p class="cart-widget__amount">{{ cartSize }}</p>
        <svg
          width="35px"
          height="35px"
          viewBox="0 0 24 24"
...
const highlightSpecial = (event) => {
  ...
  parentElement.classList.add("cart-widget-active");
  parentElement.firstChild.firstChild.classList.add(
    "cart-widget__amount-active"
  );
  parentElement.firstChild.lastChild.classList.add("cart-widget__icon-active");
};

const removeHighlight = (event) => {
  const parentElement = event.target;
  parentElement.classList.remove("cart-widget-active");
  parentElement.firstChild.firstChild.classList.remove(
    "cart-widget__amount-active"
  );
  parentElement.firstChild.lastChild.classList.remove(
    "cart-widget__icon-active"
  );
};
</script>
